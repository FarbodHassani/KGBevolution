...
int main(int argc, char **argv)
{
	...
	arr<double> ringwgt(16384, 1.0);
	std::queue<Alm<fcomplex> > almqueue;
	Healpix_Map<float> themap;
	Alm<fcomplex> cdmalm(8192,8192);
	Alm<fcomplex> rsdalm;
	double Cl[8192];
	
	for (int i = 0; i < 8192; i++) Cl[i] = 0.;
	cdmalm.SetToZero();
	...
	metric_container rsd0;
	metric_container rsd1;
	
	metric_container cdm0;
	metric_container cdm1;

	phi0.init(back[0], sim.output_path, sim.basename_lightcone, "phi");
	phi1.init(back[1], sim.output_path, sim.basename_lightcone, "phi");
	
	rsd0.init(back[0], sim.output_path, sim.basename_lightcone, "rsd");
	rsd1.init(back[1], sim.output_path, sim.basename_lightcone, "rsd");
	
	cdm0.init(back[0], sim.output_path, sim.basename_lightcone, "cdm");
	cdm1.init(back[1], sim.output_path, sim.basename_lightcone, "cdm");
	...
	std::map<int,metric_data>::iterator rsdit0;
	std::map<int,metric_data>::iterator rsdit1;
	
	std::map<int,metric_data>::iterator cdmit0;
	std::map<int,metric_data>::iterator cdmit1;
	...
	while(outcnt < numoutputs)
	{
		...
		loadHealpixData(&rsd0, tauobs - back[step].tau, tauobs - back[step + 1].tau, lightconeparam);
		loadHealpixData(&rsd1, tauobs - back[step].tau, tauobs - back[step + 1].tau, lightconeparam);
		
		loadHealpixData(&cdm0, tauobs - back[step].tau, tauobs - back[step + 1].tau, lightconeparam);
		loadHealpixData(&cdm1, tauobs - back[step].tau, tauobs - back[step + 1].tau, lightconeparam);
		...
		for (rsdit0 = rsd0.healpix_data.begin(); rsdit0 != rsd0.healpix_data.end() && rsdit0->second.hdr.distance < tauobs - back[step].tau; rsdit0++);
		for (rsdit1 = rsd1.healpix_data.begin(); rsdit1 != rsd1.healpix_data.end() && rsdit1->second.hdr.distance < tauobs - back[step].tau; rsdit1++);
		
		for (cdmit0 = cdm0.healpix_data.begin(); cdmit0 != cdm0.healpix_data.end() && cdmit0->second.hdr.distance < tauobs - back[step].tau; cdmit0++);
		for (cdmit1 = cdm1.healpix_data.begin(); cdmit1 != cdm1.healpix_data.end() && cdmit1->second.hdr.distance < tauobs - back[step].tau; cdmit1++);
...
#pragma omp parallel for
		...
		for(; it0->second.hdr.distance < tauobs - back[step+1].tau && it1->second.hdr.distance < tauobs - back[step+1].tau && dist < distances[numoutputs-1]; it0++, it1++, rsdit0++, rsdit1++, cdmit0++, cdmit1++)
		{
			if (it0 == phi0.healpix_data.end() || it1 == phi1.healpix_data.end() || rsdit0 == rsd0.healpix_data.end() || rsdit1 == rsd1.healpix_data.end() || cdmit0 == cdm0.healpix_data.end() || cdmit1 == cdm1.healpix_data.end())
			{
				cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": missing HEALPix data beyond " << dist * sim.boxsize << " Mpc/h!" << endl;
				return -1;
			}
...
#pragma omp parallel for
			...
			themap.SetNside(cdmit0->second.hdr.Nside, RING);
#pragma omp parallel for
			for (int l = 0; l < Npix_final; l++)
				themap[l] = lin_int(cdmit0->second.pixel[l],cdmit1->second.pixel[l], (cdmit0->second.hdr.distance - (tauobs - back[step].tau))/((back[step].tau - back[step+1].tau)));
				
			almqueue.push(Alm<fcomplex>(2*cdmit0->second.hdr.Nside, 2*cdmit0->second.hdr.Nside));
			
			map2alm_iter(themap, almqueue.back(), 2, ringwgt);
			
#pragma omp parallel for
			for (int l = 0; l < 2*cdmit0->second.hdr.Nside; l++)
			{
				for (int m = 0; m <= l; m++)
					cdmalm(l,m) += almqueue.back()(l,m);
			}
			
			while (almqueue.size() > 30)
			{
#pragma omp parallel for
				for (int l = 0; l < almqueue.front().Lmax(); l++)
				{
					for (int m = 0; m <= l; m++)
						cdmalm(l,m) -= almqueue.front()(l,m);
				}
				almqueue.pop();
			}
			
			if (rsdit0->second.hdr.distance >= distances[0])
			{
#pragma omp parallel for
				for (int l = 0; l < Npix_final; l++)
					themap[l] = lin_int(rsdit0->second.pixel[l],rsdit1->second.pixel[l], (rsdit0->second.hdr.distance - (tauobs - back[step].tau))/((back[step].tau - back[step+1].tau)));
					
				themap.Scale(1. - 1. / Hconf(lin_int(back[step].a,back[step+1].a, (rsdit0->second.hdr.distance - (tauobs - back[step].tau))/((back[step].tau - back[step+1].tau))), 1.5 * sim.boxsize * sim.boxsize / C_SPEED_OF_LIGHT / C_SPEED_OF_LIGHT, cosmo) / rsdit0->second.hdr.distance);
			
				rsdalm.Set(2*rsdit0->second.hdr.Nside, 2*rsdit0->second.hdr.Nside);
				
				map2alm_iter(themap, rsdalm, 2, ringwgt);

#pragma omp parallel for				
				for (int l = 0; l < 2*rsdit0->second.hdr.Nside; l++)
				{
						Cl[l] += rsdalm(l,0).real() * cdmalm(l,0).real();
						for (int m = 1; m <= l; m++)
						{
							Cl[l] += 2. * (rsdalm(l,m).real() * cdmalm(l,m).real() + rsdalm(l,m).imag() * cdmalm(l,m).imag());
						}
				}
			}
...
#pragma omp parallel for				
		...
		rsd0.cinfo = back[step];
		rsd1.cinfo = back[step+1];
		
		cdm0.cinfo = back[step];
		cdm1.cinfo = back[step+1];
	}
	...
	sprintf(filename0,"%s%s_unitycl.dat",sim.output_path,sim.basename_generic);
	clfile = fopen(filename0, "w");

	if (clfile==NULL)
	{
		cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": file " << filename0 << " cannot be opened for output!" << endl;
		return -1;
	}
