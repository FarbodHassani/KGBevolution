    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_alpha],ppt->has_source_alpha,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_alpha_prime],ppt->has_source_alpha_prime,storeidx);
...
int perturb_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"alpha",ppt->has_source_alpha);
      class_store_columntitle(titles,"alpha_prime",ppt->has_source_alpha_prime);
...
int perturb_indices_of_perturbs(
  ...
  ppt->has_source_alpha_prime = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
	...
	if (pba->has_smg == _TRUE_)
          ppt->has_source_vx_prime_smg = _TRUE_;
        if (pba->has_ur == _TRUE_)
      ...
      if ( ppt->has_metricpotential_transfers == _TRUE_ ) {
        ...
        if (ppt->gauge == synchronous) {
          ...
          ppt->has_source_alpha_prime = _TRUE_;
        }
      ...
      class_define_index(ppt->index_tp_alpha_prime,  ppt->has_source_alpha_prime, index_type,1);
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (ppt->has_source_alpha == _TRUE_)
        _set_source_(ppt->index_tp_alpha) = y[ppw->pv->index_pt_alpha];

      if (ppt->has_source_eta_prime == _TRUE_)
        _set_source_(ppt->index_tp_alpha_prime) = dy[ppw->pv->index_pt_alpha];
